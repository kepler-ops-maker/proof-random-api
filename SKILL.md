...
1. Fetch `GET https://proof-random-api.pn-26f.workers.dev/v1/random?nonce=<unique-percent-encoded-id>`; give `nonce` 1-128 characters. The Worker echoes the nonce, but the beacon itself is the latest published round and is NOT selected or changed by the nonce.
